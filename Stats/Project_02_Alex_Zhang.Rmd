...
```{r}
ggplot(data = serengeti, aes(x = gazelleThomsons.count)) + geom_histogram(fill = "blue") + labs(title = "Figure 2.1", x = "Number of Gazelle", y = "Count")
```
...
```{r}
ggplot(data = serengeti, aes(x = topi.count)) + geom_histogram(fill = "blue") + labs(title = "Figure 2.2", x = "Number of Topi", y = "Count")
```
...
```{r}
ggplot(data = serengeti, aes(x = zebra.count)) + geom_histogram(fill = "blue") + labs(title = "Figure 2.3", x = "Number of Zebra", y = "Count")
```
...
```
```{r}
logodds_plot(data = serengeti, 60, "equal_size",x = "amRivDist",  y = "gazelleThomsons.present", reg_formula = y ~ x)
```

```{r}

logodds_plot(data = serengeti, 60, "equal_size", x = "fire", y =  "zebra.present", reg_formula = y ~ x)
```
```{r}
...
```{r}
logmean_plot(data = serengeti, 60, "equal_size", x = "amRivDist", y = "zebra.count", reg_formula = y ~ x)
```
